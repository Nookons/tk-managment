...
export const AllToteToPDF = (totes: ITote[]) => {
    ...
    totes.forEach((tote, toteIndex) => {
        ...
        doc.setFontSize(12);
        doc.text(`Tote: ${tote.tote_number}`, margin, yPosition);
        yPosition += 10;
        ...
        function drawItem(item: IItem, count: number, index: number) {
            ...
            doc.setDrawColor(0);
            doc.rect(margin + 10, yPosition, pageWidth - 2 * margin, 15 + (splitText.length + splitText2.length - 2) * 5);
            doc.rect(margin + 15, yPosition + 4.5, 3.5, 3.5);

            // Print the item name and quantity
            doc.text(splitText, margin + 20, yPosition + 7);
            doc.text(splitText2, margin + 20, yPosition + 12 + (splitText.length - 1) * 5);

            yPosition += 20 + (splitText.length + splitText2.length - 2) * 5;
        }
